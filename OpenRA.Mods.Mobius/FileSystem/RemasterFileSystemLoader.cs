...
namespace OpenRA.Mods.Mobius.FileSystem
{
	public class RemasterFileSystemLoader : IFileSystemLoader, IFileSystemExternalContent
	{
		...
		public void Mount(OpenRA.FileSystem.FileSystem fileSystem, ObjectCreator objectCreator)
		{
			...
			var modID = (string)fileSystem.GetType().GetField("modID", BindingFlags.NonPublic | BindingFlags.Instance)?.GetValue(fileSystem);
			var contentSourceSettings = Game.Settings.GetOrCreate<ContentSourceSettings>(objectCreator, modID);
			if (contentSourceSettings.ContentSource == null || !contentSources.TryGetValue(contentSourceSettings.ContentSource, out var source))
				return;
		...
		bool IFileSystemExternalContent.InstallContentIfRequired(ModData modData)
		{
			...
			if (!UseRemasteredContent)
			{
				...
				foreach (var key in (string[])["Voices", "Notifications", "Music"])
				{
					...
					var files = (ImmutableArray<string>)field.GetValue(manifest);
					field.SetValue(manifest, files.Select(f => f.Replace("cnchd|", "cnc|")).ToImmutableArray());
				}
