...
{
	public class SteamContentOrigin : ContentOrigin
	{
		[FieldLoader.Require]
		[Desc("The steam app ID to query.")]
		...
		bool ResolveOrigin(OpenRA.FileSystem.FileSystem fileSystem, bool mount)
		{
			...
			{
				var manifestPath = Path.Combine(steamDirectory, "steamapps", $"appmanifest_{AppId}.acf");
				...
				var folder = new Folder(Path.Combine(steamDirectory, "steamapps", "common", installDir));
